...
#' @param path_lengths Path to optional .csv with columns "path_count", "PathName",'length', "Jminus", and "Jplus" defining the road length of each path and space between previous and next path, in meters.
...
#' @param plugning_RI Return Interval to use for deep events if no events occur in the record. Default is 1000.
...
AHI_template <- function (input_data, n_frequency = 1, major_paths = NULL, WADT = 7000,
          ...
          path_lengths = NULL, wait_time = 1, Ps = 0.05, Ps_prime = 0.15, Lave_light = 0.3,
          Lave_deep = 0.7, Lave_plunging = 0.5)
{
  library(dplyr)
  library(tidyr)
  library(reshape2)
  ...
  for (ob in 1:length(MajorPaths)) {
    ...
    storage[ob, "lastdate"] <- max(WorkingData[full_record,
                                               "Year"], na.rm = TRUE)
    storage[ob, "recordlength"] <- ifelse(storage[ob, "lastdate"] ==
    ...
    storage[ob, "ave_length_light_ft"] <- round(ifelse(length(light_events) > 0,
                                                  ifelse(all(is.na(WorkingData[light_events, "RoadLength"])),
                                                         ifelse(!is.null(path_lengths), Lave_light*path_lengths[ob, "length"], 1/.3048),
                                                         mean(WorkingData[light_events, "RoadLength"], na.rm = TRUE)),
                                                 ifelse(!is.null(path_lengths), Lave_light*path_lengths[ob, "length"], 1/.3048)), 0)
    storage[ob, "ave_length_deep_ft"] <- round(ifelse(length(deep_events) > 0,
                                                  ifelse(all(is.na(WorkingData[deep_events, "RoadLength"])),
                                                         ifelse(!is.null(path_lengths), Lave_deep*path_lengths[ob, "length"], 1/.3048),
                                                         mean(WorkingData[deep_events, "RoadLength"], na.rm = TRUE)),
                                                ifelse(!is.null(path_lengths), Lave_deep*path_lengths[ob, "length"], 1/.3048)), 0)
    storage[ob, "ave_length_plunging_ft"] <- round(ifelse(length(plunging_events) > 0,
                                                  ifelse(all(is.na(WorkingData[plunging_events, "RoadLength"])),
                                                         ifelse(!is.null(path_lengths), Lave_plunging*path_lengths[ob, "length"], 1/.3048),
                                                         mean(WorkingData[plunging_events, "RoadLength"], na.rm = TRUE)),
                                                  ifelse(!is.null(path_lengths), Lave_plunging*path_lengths[ob, "length"], 1/.3048)), 0)
    ...
    # Check and assign powder events
      ...
      if (!is.null(path_lengths) && powder[ob, "PathName"] %in% path_lengths$PathName) {
        powder[ob, "Lmax_m"] <- path_lengths[path_lengths$PathName == powder[ob, "PathName"], "length"]
      } else {
    ...
    # Check and assign light events
      ...
      if (!is.null(path_lengths) && light[ob, "PathName"] %in% path_lengths$PathName) {
        light[ob, "Lmax_m"] <- path_lengths[path_lengths$PathName == light[ob, "PathName"], "length"]
      } else {
    ...
    # Check and assign deep events
      ...
      if (!is.null(path_lengths) && deep[ob, "PathName"] %in% path_lengths$PathName) {
        deep[ob, "Lmax_m"] <- path_lengths[path_lengths$PathName == deep[ob, "PathName"], "length"]
      } else {
    ...
    # Check and assign plunging events
      ...
      if (!is.null(path_lengths) && plunging[ob, "PathName"] %in% path_lengths$PathName) {
        plunging[ob, "Lmax_m"] <- path_lengths[path_lengths$PathName == plunging[ob, "PathName"], "length"]
      } else {
      ...
      waiting[ob, "j_minus"] <- path_lengths[ob, "Jminus"]
      waiting[ob, "j_plus"] <- path_lengths[ob, "Jplus"]
      waiting[ob, "Lmax"] <- path_lengths[ob, "length"]
